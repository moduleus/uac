...
If you want to enable logging, set `URX_DEBUG` (and not `UAC_DEBUG`) environment variable to:
...
If you try to use `uac.saveToFile(...)` or `uac.loadFromFile(...)` without setting `HDF5_DISABLE_VERSION_CHECK` to `1` (warning only) or `2` (disable warning message), MATLAB will crash with error: `Headers are 1.8.23, library is 1.8.12`.
