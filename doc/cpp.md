...
You can use CMake to build uac for C++ / MATLAB / Python user.
...
If you want to have urx from vcpkg, you need to set `WITH_VCPKG_URX:BOOL=ON` CMake variable and not `URX_SRC`. See various examples in `.gitlab-ci.yml`.

If you want to have urx from submodule, you need to set `URX_SRC=urx` CMake variable and not `WITH_VCPKG_URX`. `URX_SRC` is the path of the urx folder. See `build:ubuntu-without-3rd-party` in `.gitlab-ci.yml`.
